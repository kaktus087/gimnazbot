...
btn1 = types.KeyboardButton(f"📑 Замещение")
markup.add(btn1) 
...
def start(message):
    ...
    bot.send_message(message.chat.id, text='''✏️Привет, Бот предоставляет информацию о расписании и замещении. Напиши /help для того, чтобы узнать команды''', reply_markup=markup)
        
@bot.message_handler(commands=['help'])
def help(message):
    bot.send_message(message.chat.id, text='Бот имеет 2 команды: расписание и замещение. Чтобы узнать расписание - напиши свой класс, например 11-3.', reply_markup=markup)
